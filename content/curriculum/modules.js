 ...
 * each module covers.
 *
 * `lessonRangeHint` is a *suggestion* used only by `scripts/new-lesson.js`
 * to pre-fill a sensible level/module when scaffolding a new lesson file.
 * It is never read by the build/render pipeline — see ARCHITECTURE.md
 * "Why no lesson-number routing table" for why that boundary matters as
 * the course grows past lesson 85.
 *
...
function suggestPlacement(lessonNumber) {
